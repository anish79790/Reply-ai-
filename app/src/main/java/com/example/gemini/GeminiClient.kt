...
class GeminiClient(
    ...
    suspend fun generateContent(apiKey: String, prompt: String): Result<String> = withContext(Dispatchers.IO) {
        ...
        if (trimmedKey.isBlank()) {
            return@withContext Result.failure(IllegalArgumentException("GEMINI_ERROR: Gemini API key is missing. Please enter your API key in Settings."))
        }

        var lastError: Exception? = null

        for (model in MODELS) {
            val url = "$BASE_URL/$model:generateContent?key=$trimmedKey"

            try {
                Log.d(TAG, "PROVIDER: Gemini | MODEL: $model | REQUEST START")
                ...
                Log.d(TAG, "PROVIDER: Gemini | MODEL: $model | HTTP STATUS: ${response.code} | RESPONSE RECEIVED")
                ...
                if (!response.isSuccessful) {
                    ...
                    Log.w(TAG, "Model $model returned error ($response.code): $errorMsg")
                    lastError = IllegalStateException("GEMINI_ERROR: HTTP ${response.code} - [$model] $errorMsg")
                    continue
                }
                ...
                if (candidates == null || candidates.length() == 0) {
                    lastError = IllegalStateException("GEMINI_ERROR: No response candidate from $model")
                    continue
                }
                ...
                if (replyText.isNotBlank()) {
                    ...
                    lastError = IllegalStateException("GEMINI_ERROR: Empty text returned by $model")
                }
            } catch (e: Exception) {
        ...
        Result.failure(lastError ?: IllegalStateException("GEMINI_ERROR: All Gemini models failed"))
    }
}
